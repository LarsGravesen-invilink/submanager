...
interface Subscription {
  ...
  unreadRenewalCount: number;
}

interface Report {
  id: string;
  type: "ordinary" | "renewal";
  message: string;
  createdAt: string;
  ip?: string | null;
  isRead?: boolean;
...
export default function DashboardClient({ initialCfg }: { initialCfg: Record<string, string> }) {
  ...
  const extendSubscription = async () => {
    ...
    const response = await fetch(`/api/subscriptions/${extendModal.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const openReports = async (sub: Subscription) => {
    ...
    try {
      ...
      if (unread) {
        ...
        setReports(current => current.map(report => ({ ...report, isRead: true })));
        setSubs(current => current.map(item => item.id === sub.id ? { ...item, unreadReportCount: 0, unreadOrdinaryCount: 0, unreadRenewalCount: 0 } : item));
      }
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
                      ...
                      <button onClick={() => deleteSub(sub.id, sub.name)} className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-sm font-medium bg-graphite-800 text-red-400 border border-graphite-700 hover:border-red-500/30 hover:bg-red-500/10 transition-all" title="Удалить"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                       ...
                       <button onClick={() => openReports(sub)} className={`relative inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-sm font-medium border transition-all ${sub.unreadRenewalCount > 0 ? "bg-red-500/10 text-red-400 border-red-500/30 hover:bg-red-500/20" : sub.unreadOrdinaryCount > 0 ? "bg-yellow-500/10 text-yellow-400 border-yellow-500/30 hover:bg-yellow-500/20" : "bg-graphite-800 text-graphite-400 border-graphite-700 hover:text-graphite-200"}`} title="Сообщения о проблемах"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M7 8h10M7 12h6m-8 8l2.5-3H19a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2v3z" /></svg>{sub.unreadReportCount > 0 && <span className={`min-w-5 rounded-full px-1.5 text-center text-xs text-white ${sub.unreadRenewalCount > 0 ? "bg-red-500" : "bg-yellow-500"}`}>{sub.unreadReportCount}</span>}</button>
                    </div>
                  </div>
                </div>
              );
